<?php
    function updating_to_transport($selected_name,$selected_sheet,$conn)
    {
		$flg = false;
            ...
            foreach($selected_sheet as $rowValues){
                ...
                foreach($rowValues as $key => $rowValue){
                    ...
                    if($arr[0]!="''"){
                        ...
                        $transportFormat = str_starts_with($arr[3], 'T');
                        if($flg && $transportFormat){
                            $query = "INSERT INTO transport (date, customerName, voucherType, voucherNumber, amount, validated) 
                                VALUES (".$arr[$required[0]].", ".$arr[$required[1]].", 'transport', ".$arr[$required[2]].", ".$arr[$required[3]].", '0')";
